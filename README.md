...
> A production-style single-user web application that accepts plain-text notes and web URLs, stores source content, indexes chunks using vector embeddings (Google Gemini or OpenAI), and answers questions using Retrieval-Augmented Generation (RAG) with verifiable citations.

![Stack](https://img.shields.io/badge/Stack-TypeScript%20%7C%20React%2018%20%7C%20Express%20%7C%20SQLite%20%7C%20Gemini%20%2F%20OpenAI-6366f1)
...
```
           ...
           ▼                                                                 ▼
┌───────────────────────┐                                         ┌───────────────────────┐
│ SQLite Database       │                                         │ Gemini / OpenAI API   │
│ - items (metadata)    │                                         │ - gemini-embedding-001│
│ - chunks (embeddings) │                                         │ - gemini-3.6-flash    │
└───────────────────────┘                                         └───────────────────────┘
```
...
### 2. Zero-Dependency Native SQLite Storage (`node:sqlite`)
- **Decision**: Uses Node's built-in `node:sqlite` (`DatabaseSync`) with `Float32Array` binary BLOB serialization for vector embeddings.
...
### 3. In-Memory Cosine Similarity Vector Retrieval
...
### 4. Paragraph-Aware Chunker with Fallbacks
...
### 5. Synchronous Ingestion Lifecycle
...
### 6. Strict Server-Side Request Forgery (SSRF) Protection
...
### 7. Grounded Prompting & Citation Audit Contract
...
```json
{
  ...
  "content": "AI Knowledge Inbox is built with React, Express, SQLite, and Gemini/OpenAI RAG."
}
```
...
```json
{
  "answer": "AI Knowledge Inbox is built with React, Express, SQLite, and Gemini/OpenAI RAG [1].",
  "sources": [
    {
      "citationLabel": "[1]",
      "itemId": "uuid-123",
      ...
      "sourceType": "note",
      "sourceUrl": null,
      "snippet": "AI Knowledge Inbox is built with React, Express, SQLite, and Gemini/OpenAI RAG.",
      "score": 0.892
    }
  ],
  "requestId": "req-456"
